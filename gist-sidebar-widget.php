...
/*
  ...
  Version: 1.0.1
  Author: norcross<a href="http://haoqis.com"> && Haoqis</a> 
...
class rkv_ListGistsWidget extends WP_Widget {
    ...
    function widget($args, $instance) {
        ...
        if (empty($user)) {
            ...
            $gist_list = (array) @json_decode($response['body']);
            if (false == $response || !$gist_list) {
                ...
                if (!empty($number)) {
                    ...
                    $max = 100;
                } // 100 is the max return in the GitHub API
            ...
            if (is_wp_error($response)) {
                echo '<li>Sorry, there was an error with your request.</li>';
            } else {
                $gist_list = (array) @json_decode($response['body']);
                ...
                foreach ($gist_list as $gist) {
                    ...
                    if ($date == 1) : echo '<br /><span class="gist_date">Created: ' . $create . '</span>';
                    endif;
                ...
                if ($link == 1) : echo '<p class="github_link"><a href="https://github.com/' . $user . '" title="' . $text . '" target="_blank">' . $text . '</a></p>';
                endif;
            } // end error check
    ...
    function form($instance) {
        ...
        <?php if (empty($github_user)) : echo '<span class="gist_error_message">Username is required!</span>';
        endif; ?>
        </p>
...
// class 
// register widget
add_action('widgets_init', 'rkv_ListGistsWidget_init');

function rkv_ListGistsWidget_init() {
    add_filter('https_local_ssl_verify', create_function('', 'return false;'));
    add_filter('https_ssl_verify', create_function('', 'return false;'));
